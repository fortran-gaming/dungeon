...
6350      CALL RSPSUB(961,ODI2)                  ! now tied to object.
      ...
      IF(HERE.EQ.SLIDE) OFLAG1(PRSI)=IOR(OFLAG1(PRSI), NDSCBT)
...
22000      IF(PRSO.NE.STAMP) GO TO 22100            ! stamp?
      IF(PRSA.EQ.TAKEW) OFLAG1(BROCH)=IAND(OFLAG1(BROCH), NOT(CONTBT))
      GO TO 10                        ! do normal take.
C
...
29100      IF(PRSA.NE.PUTW) GO TO 10            ! put in slide?
      IF(IAND(OFLAG1(PRSO), TAKEBT).NE.0) GO TO 29200
...
31100      IF(PRSA.NE.RINGW) GO TO 31400            ! ring?
      ...
      RETURN
C
31200      IF(IAND(OFLAG1(PRSI), BURNBT).EQ.0) GO TO 31300
...
36100      CALL RSPEAK(85+RVSND)                  ! 1..3, describe.
      RETURN
C
36400      IF(IAND(OFLAG1(STATU), VISIBT).EQ.0) CALL RSPEAK(89)
...
275      IF(XTYPE.NE.XNORM) GO TO 10            ! normal exit?
      IF(IAND(RFLAG(XROOM1), RMUNG).NE.0) GO TO 10
      BLOC=XROOM1
      CFLAG(CEVBAL)=.TRUE.
      CTICK(CEVBAL)=3
10      BALLOP=.FALSE.
