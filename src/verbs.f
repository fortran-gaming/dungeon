...
600	IF(PRSI.EQ.0) GO TO 700			! read through obj?
	IF(IAND(OFLAG1(PRSI), TRANBT).NE.0) GO TO 700
...
C V102--	Inflate
C
2000	IF(IAND(OFLAG1(PRSI), TOOLBT).NE.0) GO TO 2100	! tool?
...
C V104--	Alarm
C
4000	IF(IAND(OFLAG2(PRSO), VILLBT).NE.0) GO TO 4100	! villain?
...
C V114--	Tie
C
14000	IF(IAND(OFLAG2(PRSO), TIEBT).NE.0) GO TO 14100
...
C V115--	Tie up
C
15000	IF(IAND(OFLAG2(PRSI), TIEBT).NE.0) GO TO 15100
...
C V116--	Turn
C
16000	IF(IAND(OFLAG1(PRSO), TURNBT).NE.0) GO TO 16100
	CALL RSPEAK(390)			! not turnable.
	RETURN
C
16100	IF(IAND(OFLAG1(PRSI), TOOLBT).NE.0) GO TO 16200
...
21100	IF(PRSO.NE.AV) GO TO 21150		! vehicle?
	...
	RETURN
C
21150	IF(IAND(OFLAG1(PRSO), TAKEBT).NE.0) GO TO 21200	! takeable?
...
21300	CALL RSPSUB(397,ODO2)			! spill the works.
	DO 21500 I=1,OLNT			! spill contents.
	  ...
	  OFLAG2(I)=IOR(OFLAG2(I), TCHBT)		! touch object.
...
23000	IF(OBJACT(X)) RETURN			! object handle?
	...
	RETURN
C
23100	IF(IAND(OFLAG1(PRSO), ONBT).EQ.0) GO TO 23200
	CALL RSPEAK(401)			! already on.
	RETURN
C
23200	OFLAG1(PRSO)=IOR(OFLAG1(PRSO), ONBT)	! turn it on.
...
24000	IF(OBJACT(X)) RETURN			! obj handle?
	...
	RETURN
C
24100	IF(IAND(OFLAG1(PRSO), ONBT).NE.0) GO TO 24200
	CALL RSPEAK(403)			! already off.
	RETURN
C
24200	OFLAG1(PRSO)=IAND(OFLAG1(PRSO), NOT(ONBT))
...
25200	IF(.NOT.QOPEN(PRSO)) GO TO 25225
	...
	RETURN
C
25225	OFLAG2(PRSO)=IOR(OFLAG2(PRSO), OPENBT)	! open it.
...
26100	IF(QOPEN(PRSO)) GO TO 26200		! open?
	...
	RETURN
C
26200	OFLAG2(PRSO)=IAND(OFLAG2(PRSO), NOT(OPENBT))
...
27000	IF(OBJACT(X)) RETURN			! obj handle?
	...
	2 (.NOT.QOPEN(J).OR.(IAND(OFLAG1(J), (DOORBT+CONTBT)).EQ.0))))
...
C V130--	Board
C
30000	IF(IAND(OFLAG2(PRSO), VEHBT).NE.0) GO TO 30100
...
34000	IF(PRSI.NE.0) GO TO 34050		! any obj specified?
	IF(IAND(RFLAG(HERE), (RWATER+RFILL)).NE.0) GO TO 34025
...
41300	J=471					! assume no weapon.
	...
	IF(IAND(OFLAG2(PRSI), WEAPBT).EQ.0) GO TO 41400
...
C V149--	Save
C
49000	IF(IAND(RFLAG(TSTRS), RSEEN).EQ.0) GO TO 49100
...
C V150--	Restore
C
50000	IF(IAND(RFLAG(TSTRS), RSEEN).EQ.0) GO TO 50100
...
52200	CALL RSPSUB(525,ODO2)			! closed door- cant see.
	RETURN
C
52300	IF(IAND(OFLAG1(PRSO), CONTBT).NE.0) GO TO 52500	! container?
...
52400	IF(.NOT.QEMPTY(PRSO)) GO TO 52700	! not empty?
	...
	RETURN
C
52500	IF(QOPEN(PRSO).OR.(IAND(OFLAG1(PRSO), TRANBT).NE.0))
...
65000	IF(PRSO.NE.STRAD) GO TO 65200		! play violin?
	...
	IF(IAND(OFLAG2(PRSI), WEAPBT).EQ.0) GO TO 65100
...
65100	CALL RSPEAK(934)			! offensive noise.
	RETURN
C
65200	IF(IAND(OFLAG2(PRSO), VILLBT).EQ.0) GO TO 10	! play villain?
...
85100	I=10					! assume ok.
	...
	I=964					! joke 1
	IF(IAND(OFLAG2(PRSO), VILLBT).NE.0) I=965 ! joke 2.
85200	CALL RSPEAK(I)
	RETURN
C
C V86--	Walk through
...
86200	CONTINUE
C
86300	IF(IAND(OFLAG1(PRSO), TAKEBT).NE.0) GO TO 86400	! tkble?
	I=669					! no, joke.
...
89100	I=392					! assume tool.
	IF(IAND(OFLAG1(PRSI), TOOLBT).EQ.0) I=393
	CALL RSPSUB(I,ODI2)
	RETURN
C
C V90--	Time
...
91000	IF(PRSO.EQ.0) GO TO 91200		! over something?
	...
	RETURN
C
91100	IF(IAND(OFLAG2(PRSO), VILLBT).EQ.0) GO TO 91300
...
95000	IF(WINNER.NE.PLAYER) GO TO 10		! must do yourself.
	IF(IAND(RFLAG(MREYE), RSEEN).NE.0) GO TO 95800	! too late?
...
96600	CALL RSPEAK(827)			! quiz over,
	CFLAG(CEVINQ)=.FALSE.
	OFLAG2(QDOOR)=IOR(OFLAG2(QDOOR), OPENBT)	! open door.
	RETURN
C
...
C
	...
	1 (IAND(OFLAG2(PRSO), NOCHBT).EQ.0)) GO TO 100	! star or nocheck?
...
200	IF(PRSO.NE.AVEHIC(WINNER)) GO TO 400	! his vehicle?
	...
	RETURN
C
400	IF(IAND(OFLAG1(PRSO), TAKEBT).NE.0) GO TO 500 ! takeable?
...
100	DROP=.TRUE.				! assume wins.
	...
	IF(IAND(OFLAG2(X), OPENBT).NE.0) GO TO 300 ! is it open?
...
100	PUT=.FALSE.
	...
	RETURN
C
200	IF(QOPEN(PRSI).OR.(IAND(OFLAG1(PRSI), (DOORBT+CONTBT)).NE.0)
	1	.OR.(IAND(OFLAG2(PRSI), VEHBT).NE.0)) GO TO 300
...
400	DO 500 PRSO=1,OLNT			! loop thru objects.
	...
	1	(IAND(OFLAG2(PRSO), TRYBT).EQ.0)).OR.
	2	(IAND(OFLAG1(PRSO), VISIBT).EQ.0).OR.
	3	(IAND(OFLAG2(PRSO), ACTRBT).NE.0).OR.
	4	NOTHIS(PRSO)) GO TO 500
	  IF(.NOT.NOHERE(PRSO)) GO TO 450	! is it here?
	  J=OCAN(PRSO)				! get container.
	  IF((J.EQ.0).OR.(PRSO.EQ.WATER)) GO TO 500	! in a cont?
	  IF((IAND(OFLAG2(J), OPENBT).EQ.0).OR.
...
2400	DO 2500 PRSO=1,OLNT			! loop thru objects.
	...
	1	(NOHERE(PRSO).OR.
	2	((IAND(OFLAG1(PRSO), TAKEBT).EQ.0).AND.
	3	 (IAND(OFLAG2(PRSO), TRYBT).EQ.0)))) .OR.
	4	(PRSO.EQ.PRSI).OR.NOTHIS(PRSO).OR.
	5	(IAND(OFLAG1(PRSO), VISIBT).EQ.0)) GO TO 2500
	  F=.FALSE.
	  CALL RSPSUB(580,ODESC2(PRSO))
	  F1=PUT(.TRUE.)
	  IF(SAVEH.NE.HERE) GO TO 4500
...
525	XSTRNG=678				! assume wall.
	...
	IF((IAND(RFLAG(HERE), RNWALL).NE.0).AND.(WINNER.EQ.PLAYER))
...
3300	LITLDF=.TRUE.				! he can do it.
	...
	1	OFLAG2(DOOR)=IAND(OFLAG2(DOOR), NOT(TCHBT))
	RETURN
C
...
14000	FROBZF=.FALSE.				! asssume lose.
	...
	RFLAG(CPUZZ)=IAND(RFLAG(CPUZZ), NOT(RSEEN))	! reset seen.
