...
52500      IF(QOPEN(PRSO).OR.(IAND(OFLAG1(PRSO), TRANBT).NE.0))
     &      GO TO 52600                  ! open or see inside?
...
60100      IF(IAND(OFLAG1(PRSI), (LITEBT+FLAMBT+ONBT)).NE.
     &      (LITEBT+FLAMBT+ONBT)) GO TO 60300      ! on flame?
...
62000      IF(((PRSO.EQ.BROPE).AND.(BTIEF.NE.0)).OR.
     &  ((PRSO.EQ.ROPE).AND.(TTIE.EQ.PRSI)).OR.
     &  ((PRSO.EQ.ROPE).AND.(PRSI.EQ.RAILI).AND.DOMEF))
     &  GO TO 62100                        ! untie rope?
...
95600      CALL ENCRYP(PW(2),CH2)                  ! try both ways.
     ...
     &  GO TO 95900                        ! wrong or second use.
...
96100      IF(INQSTF.AND.(NQATT.LT.5).AND.(CORRCT.LT.3))
     &      GO TO 96200                  ! knocked, didn't lose or win?
...
400      DO 500 PRSO=1,OLNT                  ! loop thru objects.
     ...
     &      (IAND(OFLAG2(PRSO), TRYBT).EQ.0)).OR.
...
2400      DO 2500 PRSO=1,OLNT                  ! loop thru objects.
     ...
     &      (NOHERE(PRSO).OR.
     &      ((IAND(OFLAG1(PRSO), TAKEBT).EQ.0).AND.
     &       (IAND(OFLAG2(PRSO), TRYBT).EQ.0)))) .OR.
