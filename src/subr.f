...
300	I=RDESC2-HERE				! assume short desc.
	IF((FULL.EQ.0)
	1	.AND. (SUPERF.OR.((IAND(RFLAG(HERE), RSEEN).NE.0)
...
C
	...
	DO 1000 I=1,OLNT			! loop thru objects.
	  ...
	  IF((ODESCO(I).EQ.0).OR.
	1   (IAND(OFLAG2(I), TCHBT).NE.0)) GO TO 700
...
2000	CALL RSPSUB(DESC+ALSO,ODESC2(OBJ))	! print header.
	...
	1    (IAND(OFLAG2(I), TCHBT).EQ.0)) GO TO 3000
...
C
	...
	1	(IAND(OFLAG2(I), SCRDBT).NE.0)) GO TO 100
...
C
	...
	1	(IAND(OFLAG1(I), VISIBT).EQ.0).OR.(.NOT.PROB(PR,PR)))
	2	GO TO 50
...
100	  IF(IAND(OFLAG1(I), ONBT).NE.0) RETURN	! lit?
	...
	1	((IAND(OFLAG1(I), TRANBT).EQ.0).AND.
	2	(IAND(OFLAG2(I), OPENBT).EQ.0))) GO TO 1000
C
...
C
	...
	1	(IAND(OFLAG2(I), (ACTRBT+VILLBT)).EQ.0))
...
C
	...
	CALL SRAND(IOR((ISHFT(HIGH,16)+LOW), 1))
	RETURN
